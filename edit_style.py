...
from models.stylegan import get_stylegan
...
def manipulate_test(attribute, output_dir, noise_path, resolution, gan_model, latent_type):
    ...
    with tqdm(total = len(noise_paths)) as pbar:
        for noise_path in noise_paths:
            ...
            print(f"noise_torch {noise_torch.shape}")
            if latent_type == "ws":
                ws = noise_torch
            elif latent_type == "z":
                w = gan.net.mapping(noise_torch)
            ...
            if latent_type == "w":
                ...
                # for ials stylegan
                ws = g_mapping(noise_torch)
                ws = opt.truncation * w + (1 - opt.truncation) * avg_code
            ...
            for step_idx in range(step):
                ...
                # origin_img = g_style(test_torch.unsqueeze(0))
                # img = (origin_img + 1) / 2
            ...
            save_image(img_list[-1], f"{output_dir}/adv_ials_edited.jpg")
            save_image(img_list, f"{output_dir}/adv_ials_complete_transformation.jpg", nrow=7)
...
if __name__ == '__main__':
    ...
    p.add_argument("--output_dir", default="/home/FYP/limg0038/ials/images/generated")
    p.add_argument("--noise_path", help="the path of input noise file", default="/home/FYP/limg0038/ials/images/generated")
    ...
    p.add_argument("--num_of_pics", default=10, type=int)

    # for ials stylegan
    p.add_argument('--pretrain_root', type=str, default=r'/home/FYP/limg0038/ials/pretrain', help='path to the pretrain dir')
    p.add_argument('--dataset', type=str, default='ffhq', help='name of the face dataset [ffhq | celebahq]')
    p.add_argument('--truncation', type=float, default=0.5, help='truncation trick in stylegan')

    opt, _ = p.parse_known_args()

    g_mapping, g_style = get_stylegan(os.path.join(opt.pretrain_root, "stylegan", opt.dataset, "weight.pkl"), 'cuda')
    g_mapping.eval()
    g_style.eval()

    avg_code = np.load(os.path.join(opt.pretrain_root, "stylegan", opt.dataset, "avg_code.npy"))  # (512)
    avg_code = torch.from_numpy(avg_code).unsqueeze(0).to('cuda')  # (1,512)

    for img_number in range(opt.num_of_pics):
        manipulate_test(opt.attribute, f"{opt.output_dir}/set_{img_number}", f"{opt.noise_path}/set_{img_number}/ials_edited.npy", opt.resolution, opt.gan_model, opt.latent_type)
